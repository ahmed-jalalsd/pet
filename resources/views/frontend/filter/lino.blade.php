...
@section('content')
  ...
  <section class="content ">
    <ul class="products">
      @foreach ($linos as $lino)
        <li>
          <div class="product-card">
            <span class="product-info">
              <h3>{{ $lino->product_name }}</h3>
            </span>
            <span class="icon-square">
              <a href="{{ URL::route('products.single', ['products'=>$lino->id]) }}">
                 <img data-src="{{ URL::to('/public/icons/icon_object_01.svg') }}" alt="vai avanti" width="28px" height="28px">
              </a>
            </span>
            <div class="product-image">
              <img data-src="{{ URL::to('/public/images/product-feature/'.$lino->product_preview) }}" alt="{{$lino->product_name}}" width="100%" height="100%">
            </div>
          </div>
        </li>
      @endforeach
